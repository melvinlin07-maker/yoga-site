...
以下頁面為公開、可被索引的正式頁面(2026-08-27 實查,**114 個 HTML 檔案平鋪於根目錄,無子目錄**):
...
> **數量修正紀錄(2026-08-27):** 依新流程(今天寫、隔天發布)新增 `blog-is-it-frozen-shoulder.html`(五十肩誠實分流文),加入 `blog.html` 肩頸緊繃分類 + `sitemap.xml`,61→**62 篇 blog**,總數 113→**114 個 HTML**。
> ⚠️ **另有 `blog-arm-flab-rounded-shoulders.html`(蝴蝶袖)已寫完但刻意尚未上架**,不計入上述正式頁面數,見 `DECISIONS.md` 相關決策記錄。
...
**Sitemap 收錄範圍(2026-08-27 更新):**
- 共 **106 條 `<loc>`** = 114 個 HTML − 8 個隱藏/工具頁;首頁以 `/` 形式收錄
- 62 篇 blog + 27 篇 pose **全數在列**(2026-08-27 上架 `blog-is-it-frozen-shoulder.html` 後已同步)
